 ...
 */
uint8_t ok(uint8_t n, uint8_t a[N]) {
...
#pragma HLS INTERFACE mode = m_axi port = mem
  ...
  uint8_t l_a[N];
  for (int i = 0; i < j; i++) {
    l_a[i] = MEM_ARR_IN(mem, a, i, uint8_t);
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int k = 0; k < j; k++) {
      MEM_ARR_OUT(mem, b, k, uint8_t, l_a[k]);
    }
  ...
  io_section3 : {
    MEM_OUT(mem, task.ret_addr, uint32_t, 0);
    ap_wait_until(MEM_IN(mem, task.ret_addr, uint32_t) == 0);
    argOut.write(task.cont);
  }
    return;
  }
