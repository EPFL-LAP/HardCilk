...
void readVertex(void * mem0,
    ...
    for(int i = 0; i < v_size; i++){
      ...
      uint32_t u = MEM_ARR_IN(mem0, v_neighbors, i, uint32_t);
      
      Addr u_neighbors = MEM_ARR_IN(mem1, adj_list, u * 2, Addr);
      uint32_t u_size =  MEM_ARR_IN(mem2, adj_list, u*2 + 1, uint64_t);
...
void readMemV(hls::stream<uint32_t> &elements_v,
          ...
          for(int j = 0; j < v_size; j++){
            ...
            if(u_size > 0){
              for(int i = 0; i < v_size; i++){
                #pragma HLS PIPELINE 
                uint32_t read_v = MEM_ARR_IN(mem_v, v_neighbors, i, uint32_t);
      ...
      for(int j = 0; j < v_size; j++){
        ...
        if(u_size > 0){
          for(int i = 0; i < u_size; i++){
            #pragma HLS PIPELINE 
            uint32_t read_u = MEM_ARR_IN(mem_u, u_neighbors, i, uint32_t);
...
void readTask(void * mem, 
        ...
        Addr v_neighbors = MEM_ARR_IN(mem, task.adj_list, task.vertex * 2, Addr);
        
        uint32_t v_size_neighbors = MEM_IN(mem, task.adj_list + task.vertex * 8 * 2 + 8, uint64_t);
...
void writeArg(hls::stream<uint32_arg_out> &argDataOut, hls::stream<uint64_t> &argOut,  hls::stream<uint32_t> &count, hls::stream<Addr> & triangleCountEntries, hls::stream<Addr> & return_address_queue, hls::stream<bool> & fastFinishDelievery){
    ...
    uint32_arg_out argOutValues;
    argOutValues.addr = entry;
    argOutValues.allow = 1;
    argOutValues.data = count_local;
    argOutValues.size = 2;    

    argDataOut.write(argOutValues);    
...
void vertex_map(
                ...
                hls::stream<uint32_arg_out> &argDataOut) {
...
#pragma HLS DATAFLOW
  ...
  hls_thread_local hls::task t5 (writeArg, argDataOut, argOut, count_stream, triangleCountEntries, return_address_queue, fastFinishDelievery);
}
...
#pragma HLS PIPELINE off
  ...
  for(int i = 0; i < task.vertex_count; i++){
    ...
    new_task.vertex = i;
    new_task.triangle_count_entry = task.triangle_count_arr + (i * sizeof(uint32_t));
    new_task.return_address = task.cont;
    if(MEM_IN(mem, task.adj_list + (i * 2 * 8) + 8, uint64_t) > 0)
