// ─────────────────────────────────────────────────────────────────────────────
// tester.cpp — functional-accuracy CPU test for the HardCilk BFS kernels.
//
// This is NOT an FPGA / Vitis run. It links the *real* HLS C++ from ../BFS.cpp
// (the BFS orchestration kernel + sparse_edgemap_helper) and drives them from a
// plain CPU harness, so `make test` validates the actual synthesizable code for
// correctness without any Vitis tooling.
//
// What it models (mirrors xclbin-workspace/.../BFSDriver.h, but for one PE):
//   * a single flat byte buffer standing in for HBM; every "address" is a byte
//     offset into it, exactly as the kernels' MEM_* pointer arithmetic expects;
//   * the CSR graph + work buffers laid out byte-identically to the driver;
//   * the HardCilk framework's job (SchedulerServer + ArgumentNotifier): seed
...
//   * the LockServer: with ONE PE there is no contention, so the lock is a
//     trivial always-succeeds responder that just applies the requested atomic
//     read-modify-write to the byte buffer. It runs on its own thread purely so
//     the real helper's toLock/fromLock request/response handshake can complete.
//
...
// Which kernel to drive. Define BFS_NEW (via the Makefile's MODEL=new) to link
// ../BFS_new.cpp instead of ../BFS.cpp. The orchestration kernel (BFS) and the
// task/continuation interface are identical between the two; the only
// difference is the edge-map helper, which in BFS_new takes 6 m_axi ports and
// TWO independent lock servers (one for the Visited test-and-set, one for the
// nextFChar reservation counter). Those two never address the same word, so the
// harness just runs the trivial lockServer on two threads over the same buffer.

// Defined in ../BFS.cpp / ../BFS_new.cpp (the real synthesizable kernels).
...
static void referenceBFS(const Graph &G, int source, int max_depth,
                         std::vector<int> &dist) {
  ...
  while (!q.empty()) {
    ...
    q.pop();
    if (dist[u] >= max_depth)
      continue; // children would exceed max_depth
...
struct Hbm {
  ...
  template <typename T> T *ptr(uint64_t addr) {
...
static void lockServer(hls::stream<lock_req> *toLock,
  ...
  for (;;) {
    ...
    } else if (op == LOCK_OP_ADD_ONE_RETURN_CURRENT) {
      // Reserve a unique next-frontier slot: return the current counter, then
      // advance it by the operand (always 1 here).
      uint64_t *p = reinterpret_cast<uint64_t *>(mem + addr);
      prev = *p;
      *p = prev + value;
    } else {
...
int main(int argc, char **argv) {
  ...
  hls::stream<BFS_args> bfs_in("bfs_in");
  ...
  while (!bfs_in.empty()) {
    BFS(hbm.base(), helper_tasks, bfs_in); // consumes one BFS task
    rounds++;
...
#else
      sparse_edgemap_helper(hbm.base(), hbm.base(), hbm.base(), helper_in,
                            argOut, toLock, fromLock);
#endif
      helpers_run++;

      // ArgumentNotifier: each finished helper decrements the continuation's
      // join counter (field 0 of BFS_args at its cont address).
  ...
  referenceBFS(G, source, max_depth, dist_ref);
