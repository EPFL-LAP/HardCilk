...
#define N_taskAdder_cont0 2
#define N_memReader 2
#define N_taskInitiator_reentry0 2
...
// N_ defines), so the total must fit the bundle. Edit the N_ defines (and the JSON
...
// Keep each 2-bit status pin behind its own tiny stream tap. Aggregate taps look
// sensible in C++, but HLS can load the concatenated status once in FSM state1 and
// replay that register forever. Per-pin taps generate live stream inputs, like
// watcher_gate_tap, while unrolled tap arrays still allow each task's PE count to vary.
void watcher_status2_tap(
    ap_uint<2> &status,
    hls::stream<ap_uint<2>> &status_stream);
...
#pragma HLS stream variable = adder_status_in_stream depth = 256
#pragma HLS stream variable = adder_status_out_stream depth = 256
...
#pragma HLS stream variable = initiator_status_in_stream depth = 256
#pragma HLS stream variable = initiator_status_out_stream depth = 256
    ...
    hls::stream<ap_uint<2>> &status_stream)
{
  ...
  {
#pragma HLS pipeline II = 1
    volatile ap_uint<2> *status_v = &status;
    ap_uint<2> raw = *status_v;
