...
// For WidestPath, all distances should be initialized to -INF (float), so the
// first relaxation of any reachable vertex always stores a wider value; the
// source distance is then set to +INF below.
...
void WidestPath(void *mem_0, hls::stream<sparse_edgemap_helper_args> &taskOutGlobal,
...
#pragma HLS INTERFACE mode = m_axi port = mem_0 bundle = gmem channel = \
    ...
    if (first_iteration)
    {
        MEM_ARR_OUT(mem_0, task.distance, task.source, float, std::numeric_limits<float>::infinity());
...
#pragma HLS pipeline II = 1
        ...
        float new_distance = fminf(weight_and_neighbor.distance, weight_and_neighbor.vertex_current_distance);
...
#pragma HLS pipeline II = 1
        ...
        // widened this neighbor's distance, so it belongs in the next frontier.
