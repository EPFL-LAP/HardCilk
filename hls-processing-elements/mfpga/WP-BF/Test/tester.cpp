// Functional-accuracy CPU test for the HardCilk Widest Path kernels.
//
// This links the real synthesizable code from ../WidestPath.cpp and drives it
// from a plain CPU harness. The CPU golden is a directed weighted widest path:
// each vertex gets the maximum bottleneck capacity from the source, while
...
static void referenceWidestPath(const Graph &G, uint32_t source,
  ...
  for (uint32_t pass = 0; pass + 1 < n; pass++)
  {
    ...
    for (size_t i = 0; i < G.edges.size(); i++)
    {
      ...
      if (std::isinf(width[e.src]) && width[e.src] < 0.0)
        continue;
      const double candidate = std::min(width[e.src], (double)e.weight);
      if (candidate > width[e.dst] + EPS)
      {
                       ...
                       hls::stream<lock_resp> *fromLock, uint8_t *mem)
{
  for (;;)
  {
    ...
    if (op == LOCK_OP_SET_IF_LESS_AND_RETURN_CURRENT ||
        op == LOCK_OP_SET_IF_GREATER_AND_RETURN_CURRENT)
    {
      if (mode == ATOMIC_MODE_WORD)
      {
        ...
        {
          should_write =
              (op == LOCK_OP_SET_IF_LESS_AND_RETURN_CURRENT)
...
int main(int argc, char **argv)
{
  ...
  referenceWidestPath(G, source, dist_ref);
  ...
  hls::stream<BFS_args> wp_in("wp_in");
  ...
  while (!wp_in.empty())
  {
    ...
    WidestPath(hbm.base(), helper_tasks, wp_in);
    rounds++;
  ...
  std::cout << "[tester] WidestPath rounds=" << rounds
  ...
  uint32_t reached = 0, source_width = 0, unreachable = 0;
  for (uint32_t v = 0; v < n; v++)
  {
  ...
  std::cout << "[tester] reference finite=" << reached
            << " source_inf=" << source_width << " unreachable=" << unreachable
            << std::endl;

  if (!watchdog && mismatches == 0)
  {
    std::cout << "[tester] PASS: kernel distances match Widest Path golden."
              << std::endl;
    return 0;
  }
