...
uint32_t count_intersection(void * mem, Addr v_neighbors, Addr u_neighbors, uint32_t v_size, uint32_t u_size){
  ...
  #pragma HLS ARRAY_PARTITION variable=u_buffer complete 
  ...
  while((u_pointer < u_size && v_pointer < v_size)){
    ...
    for(int j = 0; (j < LOCAL_BUFFERS_SIZE); j++){
      #pragma HLS UNROLL
      //#pragma HLS PIPELINE II=1
...
#pragma HLS PIPELINE  
  ...
  Addr u_neighbors_list[LOCAL_BUFFERS_SIZE];
  
  // Find the intersections of u neighbours and v neighbours
  for (uint32_t i = 0; i < v_size_neighbors; i+=LOCAL_BUFFERS_SIZE) {
    for(int j = 0; j < LOCAL_BUFFERS_SIZE && (i+j) < v_size_neighbors; j++){
      u_list[j] = MEM_ARR_IN(mem, v_neighbors, i + j + 1, uint32_t);
      u_neighbors_list[j] = MEM_ARR_IN(mem, adj_list, u_list[j], Addr);
      u_size_neighbors_list[j] = MEM_ARR_IN(mem, u_neighbors_list[j], 0, uint32_t);

    }

    for(int j = 0; j < LOCAL_BUFFERS_SIZE && (i+j) < v_size_neighbors; j++){
      count += count_intersection(mem, v_neighbors, u_neighbors_list[j], v_size_neighbors, u_size_neighbors_list[j]);
    }
