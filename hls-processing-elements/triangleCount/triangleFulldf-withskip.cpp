...
void readMemV(hls::stream<uint32_t> &elements_v,
              ...
              hls::stream<bool> & processingDoneToReadV,
              hls::stream<int> & doneIndexV
      ){
          ...
          for(int j = 0; j < v_size; j++){
            ...
            if(u_size > 0){
              //hls::print("We got a u_size greater than 0 in read VVV\n"); 
              volatile bool terminate = false;
              for(int i = 0; i < v_size; i++){
                #pragma HLS PIPELINE 
                uint32_t read_v = MEM_ARR_IN(mem_v, v_neighbors, i + 1, uint32_t);
                elements_v.write(read_v);  
                if(!processingDoneToReadV.empty()){
                  auto indx = doneIndexV.read();
                  if(indx != j){
                    processingDoneToReadV.read();
                    continue;
                  }
                  else{
                    terminate = processingDoneToReadV.read();
                    elements_v.write(0xFFFFFFFF);
                    break;
                  }
                }
              }
              ...
              // hls::print("We finished the for loop for u_size in read VVV, Terminate %d\n", terminate);
              // // Termination Packet
              // elements_v.write(0xFFFFFFFF);
              // hls::print("We Wrote the final packet in read VVV\n"); 
              // // if(terminate == false){
              // //   processingDoneToReadV.read();                
              // // } 
              // bool temp;
              // processingDoneToReadV.read_dep(temp, terminate);
              // hls::print("We flushed the terminate packet if that was necessary in read VVV\n"); 
            }
          }
}
...
void readMemU(hls::stream<uint32_t> &elements_u,
              ...
              hls::stream<Addr> & u_neighbors_fifo,
              hls::stream<uint32_t> & u_size_fifo,
              hls::stream<bool> & processingDoneToReadU,
              hls::stream<int> & doneIndexU
      ){  
      uint32_t v_size = v_size_s.read();
      for(int j = 0; j < v_size; j++){
        #pragma HLS PIPELINE 
        ...
        if(u_size > 0){
          ...
          //hls::print("We got a u_size greater than 0 in read UUU\n"); 
          volatile bool terminate = false;
          for(int i = 0; i < u_size; i++){
            ...
            if(!processingDoneToReadU.empty()){
              auto indx = doneIndexU.read();
              if(indx != j){
                    processingDoneToReadU.read();
                    continue;
              } else{
                terminate = processingDoneToReadU.read();
                elements_u.write(0xFFFFFFFF);
                break;
              }
            }           
          }
          ...
          // hls::print("We finished the for loop for u_size in read UUU, Terminate %d\n", terminate); 
          // // Termination Packet
          // elements_u.write(0xFFFFFFFF);
          // // if(terminate == false){
          // //   processingDoneToReadU.read();
          // // }
          // hls::print("We Wrote the final packet in read UUU\n"); 
          // bool temp;
          
          // processingDoneToReadU.read_dep(temp, terminate);
          // hls::print("We flushed the terminate packet if that was necessary in read UUU\n"); 
...
void processData( hls::stream<uint32_t> &elements_v, 
    ...
    for(int j = 0; j < v_size; j++){
      ...
      if(u_size > 0){
        ...
        if((v_pointer + elements_v.size()) < v_size){
          processingDoneToReadV.write(true);
          doneIndexV.write(j);
        }
        
        
        if((u_pointer + elements_u.size()) < u_size){
          processingDoneToReadU.write(true);
          doneIndexU.write(j);
        }
        


        auto val_v = elements_v.read();
        // hls::print("val_v: %d\n", val_v);        
        while(val_v != 0xFFFFFFFF){
          #pragma HLS PIPELINE II=1 
          val_v = elements_v.read();
          //hls::print("val_v: %d\n", val_v);
        }

        auto val_u = elements_u.read();
        //hls::print("val_u: %d\n", val_u);
        while(val_u != 0xFFFFFFFF){
          #pragma HLS PIPELINE II=1 
          val_u = elements_u.read();
          //hls::print("val_u: %d\n", val_u);
        }
      }
    }
...
#pragma HLS DATAFLOW
  ...
  hls_thread_local hls::stream<uint32_t, 32> u_size_fifo_readMemU;
  ...
  hls_thread_local hls::stream<Addr, 32> addresses_neighbours_0;
  hls_thread_local hls::stream<Addr, 32> addresses_neighbours_1;
  hls_thread_local hls::stream<Addr, 32> addresses_adj_lists;
  hls_thread_local hls::stream<uint32_t, 32> sizes_0;
  hls_thread_local hls::stream<uint32_t, 32> sizes_1;
  hls_thread_local hls::stream<uint32_t, 32> sizes_2;
  hls_thread_local hls::stream<uint32_t, 32> sizes_3;
  hls_thread_local hls::stream<Addr, 33> triangleCountEntries;
  hls_thread_local hls::stream<Addr, 33> return_address_queue;
  hls_thread_local hls::stream<uint32_t, 32> count_stream;
  hls_thread_local hls::stream<bool, 33> fastFinishDelievery;

  hls_thread_local hls::stream<bool, 9> processingDoneToReadU;
  hls_thread_local hls::stream<bool, 9> processingDoneToReadV;
  hls_thread_local hls::stream<int, 9>  doneIndexV;
  hls_thread_local hls::stream<int, 9>  doneIndexU;
  ...
  hls_thread_local hls::task t2 (readMemU, elements_u_fifo, mem_4, sizes_1, u_neighbors_fifo, u_size_fifo_readMemU, processingDoneToReadU, doneIndexU);

  hls_thread_local hls::task t3 (readMemV, elements_v_fifo, mem_5, addresses_neighbours_1, sizes_2, u_size_fifo_readMemV, processingDoneToReadV, doneIndexV);

  hls_thread_local hls::task t4 (processData, elements_v_fifo, elements_u_fifo, sizes_3, u_size_fifo_processData, count_stream, processingDoneToReadV, processingDoneToReadU, doneIndexV, doneIndexU);
