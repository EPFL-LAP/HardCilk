...
int sc_main(int argc, char **argv)
{
...

    {
        ...
        if (auto it = args.find("exp2_baseDepth"); it != args.end())
        {
            _exp2_baseDepth = static_cast<uint32_t>(std::stoul(it->second));
        }

        if (auto it = args.find("exp2_branchFactor"); it != args.end())
        {
            _exp2_branchFactor = static_cast<uint32_t>(std::stoul(it->second));
        }

        if (auto it = args.find("exp2_initCount"); it != args.end())
        {
            _exp2_initCount = static_cast<uint32_t>(std::stoul(it->second));
        }

        if (auto it = args.find("exp2_delay"); it != args.end())
        {
            _exp2_delay = static_cast<uint32_t>(std::stoul(it->second));
        }
    }
