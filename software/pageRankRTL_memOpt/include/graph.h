...
struct Graph {
    ...
    static Graph loadFile(const char* fpath, bool directed = true) {
        ...
        int64_t max = -1, counter = 0;

        {
            std::ifstream ifs { fpath };

            if (!ifs)
                std::cout << "could not open the file: " << fpath << "\n";
            ...
            std::cout << "read: " << counter << " edges and " << " the maximum node index is " << max << "\n";
        }
    ...
    void writeDirect(uint8_t* mem, uint64_t base, uint64_t& ptrGraph, uint64_t& ptrAdjList) {
        ...
        for (auto const& adjList : adjLists_) {
            ...
            // align(128, 1ull << 14, base, sz, offset);
            align(128, 1ull << 14, base, sz, offset);
