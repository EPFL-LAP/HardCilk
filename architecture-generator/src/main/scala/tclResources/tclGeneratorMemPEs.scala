...
class tclGeneratorMemPEs(val fullSysGenDescriptor: fullSysGenDescriptor, val tclFileDirectory: String){
    private def initialize()={
        ...
        val tclGeneralConfigsObject = new tclGeneralConfigs()
        ...
        for (interconnectDescriptor <- memConnectionsStats.interconnectDescriptors){
            ...
            for(i <-0 until interconnectDescriptor.count){
                ...
                for (task <- fullSysGenDescriptor.taskDescriptors){
                    for (i <- taskPEIndexMap(task.name) until task.numProcessingElements){
                        if (interconnectMap(task.name) > 0 && possibleConnections > 0){
                            tclWriteln(f"connect_bd_intf_net [get_bd_intf_pins ${fullSysGenDescriptor.name}_0/${task.name}_${taskPEIndexMap(task.name)}_m_axi_gmem] [get_bd_intf_pins smartconnect_${index}/S${(possibleConnections-1)}%02d_AXI]")
                ...
                if (possibleConnections > 0 && !xdma_connected){
                    ...
                    tclWriteln(f"connect_bd_intf_net  [get_bd_intf_pins smartconnect_${index}/S${(possibleConnections-1)}%02d_AXI] [get_bd_intf_pins axi_clock_converter_0/M_AXI]")
        ...
        assert(totalConnections == memConnectionsStats.totalAXIPorts, f"Not all connections were made, only ${totalConnections} out of ${memConnectionsStats.totalAXIPorts} were made")
        ...
        tclWriteln("connect_bd_intf_net [get_bd_intf_pins axi_clock_converter_1/M_AXI] [get_bd_intf_pins axi_dwidth_converter_0/S_AXI]")
        ...
        tclWriteln(f"assign_bd_address -target_address_space /xdma_0/M_AXI_LITE [get_bd_addr_segs ${fullSysGenDescriptor.name}_0/s_axil_mgmt_hardcilk/reg0]")
...
object generateTclMemPEs extends App {
 
    val tclFileDirectory = "output"
    val pathInputJsonFile = "taskDescriptors/fibonacci.json"

    new tclGeneratorMemPEs(parseJsonFile[fullSysGenDescriptor](pathInputJsonFile), tclFileDirectory)
}