...
/** A trait that encapsulates the mfPGA-specific extensions to the Scheduler
  * module.
  */
trait SchedulerHasMfpgaSupport extends Module {
  ...
  val fpgaCountInputReg =
    if (mfpgaSupport)
      Some(IO(Input(UInt(32.W)).suggestName("fpgaCountInputReg")))
    else None
  val fpgaIndexInputReg =
    if (mfpgaSupport)
      Some(IO(Input(UInt(32.W)).suggestName("fpgaIndexInputReg")))
    else None
  val numberOfTasksToMove =
    if (mfpgaSupport)
      Some(IO(Input(UInt(32.W)).suggestName("numberOfTasksToMove")))
    else None
  val m_axis_remote =
    if (mfpgaSupport)
      Some(IO(axi4s.Master(axisCfgTaskAndReq)).suggestName("m_axis_remote"))
    else None
  val s_axis_remote =
    if (mfpgaSupport)
      Some(IO(axi4s.Slave(axisCfgTaskAndReq)).suggestName("s_axis_remote"))
    else None

  val s_axi_remote_task_server =
    if (mfpgaSupport)
      Some(IO(axi4.lite.Slave(axi4.Config(wAddr = 6, wData = 64, lite = true))))
    else None
  ...
  def buildMfpgaConnections(): Unit = {
    ...
    if (mfpgaSupport) {
      ...
      val serveRemoteAggregate =
        schedulerServers.map(_.io.serveRemote).reduce(_ || _)
      val getTasksFromRemoteAggregate = schedulerServers
        .map(_.io.getTasksFromRemote)
        .reduce(_ || _) && !serveRemoteAggregate

      if (taskWidth >= 512) {
        val axisWidth = 1024
        val remoteTaskServer = Module(
          new RemoteTaskServer_1024(
            taskWidth,
            peCount,
            axisCfgTaskAndReq.copy(wData = axisWidth),
            taskId
          )
        )
        ...
        remoteTaskServer.io.connNetwork_0 <> stealNW_TQ.io.connVSS(
          schedulerServersNumber
        )
        remoteTaskServer.io.connNetwork_1 <> stealNW_TQ.io.connVSS(
          schedulerServersNumber + 1
        )
        ...
        val fromNetworkConverter = Module(
          new Axis512To1024(
            axisCfgTaskAndReq,
            axisCfgTaskAndReq.copy(wData = axisWidth)
          )
        ...
        elastic.SourceBuffer(
          s_axis_remote.get.asFull
        ) <> fromNetworkConverter.io.in.asFull
        val toNetworkConverter = Module(
          new Axis1024To512(
            axisCfgTaskAndReq.copy(wData = axisWidth),
            axisCfgTaskAndReq
          )
        ...
        elastic.SinkBuffer(
          m_axis_remote.get.asFull
        ) <> toNetworkConverter.io.out.asFull

      } else {
        val axisWidth = 512
        val remoteTaskServer = Module(
          new RemoteTaskServer_512(
            taskWidth,
            peCount,
            axisCfgTaskAndReq.copy(wData = axisWidth),
            taskId
        ...
        elastic.SinkBuffer(
          m_axis_remote.get.asFull
        ) <> remoteTaskServer.io.m_axis_taskAndReq.asFull
        elastic.SourceBuffer(
          s_axis_remote.get.asFull
        ) <> remoteTaskServer.io.s_axis_taskAndReq.asFull
      }
