...
class taskRequestReplyType_1024(taskWidth: Int) extends Bundle {
  val fpgaId = UInt(4.W)
  val taskId = UInt(4.W)
  val globalFunctionId = globalFunctionIds()
  val isRequest = Bool()
...
class RemoteTaskServer_1024(
  ...
  queueForReceivingTasks.io.enq.bits := Cat(taskWaitingSlave.TDATA.asTypeOf(new taskRequestReplyType_1024(taskWidth)).taskData_upper,
                                           taskWaitingSlave.TDATA.asTypeOf(new taskRequestReplyType_1024(taskWidth)).taskData_lower) 
  ...
  when(fetchingRemote && networkWriterReady && !requestSent) {
    ...
    val taskRequestPacket = Wire(new taskRequestReplyType_1024(taskWidth))
  ...
  val taskReplyPacket = Wire(new taskRequestReplyType_1024(taskWidth))
  ...
  when(servingRemote && !activelyServing) {
    ...
    when(taskRequestListener.asFull.valid) {
      ...
      fpgaToServe := taskRequestListener.TDATA.asTypeOf(new taskRequestReplyType_1024(taskWidth)).fpgaId
  ...
  val s_axis_taskAndReqMarked = Wire(Irrevocable(new chext.amba.axi4s.FullChannel(io.s_axis_taskAndReq.cfg.copy(wData = 8 + new taskRequestReplyType_1024(taskWidth).getWidth))))
  ...
  new elastic.Fork(s_axis_taskAndReqMarked) {
    override def onFork(): Unit = {
      ...
      val taskRequest = Wire(new taskRequestReplyType_1024(taskWidth))
      taskRequest := in.data(new taskRequestReplyType_1024(taskWidth).getWidth - 1, 0).asTypeOf(new taskRequestReplyType_1024(taskWidth))
  ...
  when(requestForwardSlave.asFull.fire && requestForwardSlave.TDATA.asTypeOf(new taskRequestReplyType_1024(taskWidth)).fpgaId === shiftedFpgaIndexInputReg){
  ...
  when(taskRequestingMaster.asFull.valid) {
    printf("TIME %d, FPGA ID %d, packet FPGAID %d: Request generated\n", cycles_counter, shiftedFpgaIndexInputReg, taskRequestingMaster.TDATA(new taskRequestReplyType_1024(taskWidth).getWidth - 1 - 4, new taskRequestReplyType_1024(taskWidth).getWidth - 1 - 11))
  }

  when(requestForwardSlaveSupressSelfRequest.asFull.valid) {
    printf("TIME %d, FPGA ID %d, packet FPGAID %d: Request subpressed\n", cycles_counter, shiftedFpgaIndexInputReg, requestForwardSlaveSupressSelfRequest.TDATA(new taskRequestReplyType_1024(taskWidth).getWidth - 1 - 4, new taskRequestReplyType_1024(taskWidth).getWidth - 1 - 11))
  }
...
class taskRequestReplyType_512(taskWidth: Int) extends Bundle {
  val fpgaId = UInt(4.W)
  val taskId = UInt(4.W)
  val globalFunctionId = globalFunctionIds()
  val isRequest = Bool()
  ...
  val widths = fpgaId.getWidth + taskId.getWidth + globalFunctionId.getWidth + isRequest.getWidth + taskData.getWidth

  val padding_0 = UInt((512 - widths).W)

  assert(this.getWidth == 512, "taskRequestReplyType width must be multiple of 8 bits") 
}
...
class RemoteTaskServer_512(
  ...
  queueForReceivingTasks.io.enq.bits := taskWaitingSlave.TDATA.asTypeOf(new taskRequestReplyType_512(taskWidth)).taskData
  ...
  when(fetchingRemote && networkWriterReady && !requestSent) {
    ...
    // Create a request packet using the new taskRequestReplyType bundle
    val taskRequestPacket = Wire(new taskRequestReplyType_512(taskWidth))
  ...
  // pack the tak reply using the new taskRequestReplyType bundle
  val taskReplyPacket = Wire(new taskRequestReplyType_512(taskWidth))
  taskReplyPacket.fpgaId := shiftedFpgaIndexInputReg(3,0)
  taskReplyPacket.taskId := taskIndex.U(4.W)
  taskReplyPacket.globalFunctionId := globalFunctionIds.scheduler
  taskReplyPacket.isRequest := false.B
  ...
  when(servingRemote && !activelyServing) {
    ...
    when(taskRequestListener.asFull.valid) {
      // TDATA from 512 - 4 till 512-12 is the fpga index
      fpgaToServe := taskRequestListener.TDATA.asTypeOf(new taskRequestReplyType_512(taskWidth)).fpgaId
      activelyServing := true.B
    }.elsewhen(!io.serveRemote) {
      servingRemote := false.B // fall back if there was no request and the serveRemote is low
    }
  ...
  val s_axis_taskAndReqMarked = Wire(Irrevocable(new chext.amba.axi4s.FullChannel(io.s_axis_taskAndReq.cfg.copy(wData = 8 + new taskRequestReplyType_512(taskWidth).getWidth))))
  ...
  new elastic.Fork(s_axis_taskAndReqMarked) {
    override def onFork(): Unit = {
      // Create a wire to hold the parsed packet
      val taskRequest = Wire(new taskRequestReplyType_512(taskWidth))
      taskRequest := in.data(new taskRequestReplyType_512(taskWidth).getWidth - 1, 0).asTypeOf(new taskRequestReplyType_512(taskWidth))
  ...
  when(requestForwardSlave.asFull.fire && requestForwardSlave.TDATA.asTypeOf(new taskRequestReplyType_512(taskWidth)).fpgaId === shiftedFpgaIndexInputReg){
    self_requests_reforwarded := self_requests_reforwarded + 1.U
  }

  // Log request generation and request subression
  when(taskRequestingMaster.asFull.valid) {
    printf("TIME %d, FPGA ID %d, packet FPGAID %d: Request generated\n", cycles_counter, shiftedFpgaIndexInputReg, taskRequestingMaster.TDATA(new taskRequestReplyType_512(taskWidth).getWidth - 1 - 4, new taskRequestReplyType_512(taskWidth).getWidth - 1 - 11))
  }

  when(requestForwardSlaveSupressSelfRequest.asFull.valid) {
    printf("TIME %d, FPGA ID %d, packet FPGAID %d: Request subpressed\n", cycles_counter, shiftedFpgaIndexInputReg, requestForwardSlaveSupressSelfRequest.TDATA(new taskRequestReplyType_512(taskWidth).getWidth - 1 - 4, new taskRequestReplyType_512(taskWidth).getWidth - 1 - 11))
  }
