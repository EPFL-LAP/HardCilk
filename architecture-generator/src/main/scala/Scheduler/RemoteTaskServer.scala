...
class RemoteTaskServer(
  ...
  writeTaskToNetwork.numTasksToStealOrServe := numTasksToStealOrServe.U
  ...
  val queueForReceivingTasks = Module(new Queue(UInt(512.W), numTasksToStealOrServe))
  ...
  when(requestSent && dataReceievedCount < numTasksToStealOrServe.U) {
    when(dataReceievedCount === numTasksToStealOrServe.U - 1.U && taskWaitingSlave.asFull.valid && queueForReceivingTasks.io.enq.ready) {
      requestSent := false.B
  ...
  val queueForSendingTasks = Module(new Queue(UInt(512.W), numTasksToStealOrServe))
  ...
  when(fetchLocalNetwork && servingCount < numTasksToStealOrServe.U) {
    when(servingCount === numTasksToStealOrServe.U - 1.U && taskServingMaster.TREADY && queueForSendingTasks.io.deq.valid) {
