...
class RVtoAXIBridge(
    ...
    read: Boolean = true,
    write: Boolean = true,
    strbCntrl: Boolean = false,
    burstLength: Int = 0,
    varBurst: Boolean = false
) extends Module {
  ...
  val io = IO(new RVtoAXIBridgeIO(taskWidth, addrWidth, read, write, varBurst))
