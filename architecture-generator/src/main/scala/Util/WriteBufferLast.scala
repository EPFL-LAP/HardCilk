...
class WriteBundleLast(
    cfg: WriteBufferLastConfig
) extends Bundle {
  import cfg._

  private val totalSize = wAddr + wData + 32
...
class WriteBufferLast(
    cfg: WriteBufferLastConfig
) extends Module {
  import cfg._
  private val wb_t = new WriteBundleLast(cfg)
  ...
  when(s_pkg_.fire) {
    printf("WriteBuffer: addr = %x, data = %x, size = %x, allow = isLast", s_pkg_payload.addr, s_pkg_payload.data, s_pkg_payload.size)
    printf("\n")
  }
  ...
  for (i <- 0 until nAllow) {
    val counter = Module(new chext.util.Counter(cfg.nOutstanding + 1))
    counter.io.incEn := duplB(i).fire
    duplB(i).ready := duplB(i).valid & counter.notFull

    counter.noDec()
    new elastic.Arrival(s_allows_(i), m_allows_(i)) {
      protected def onArrival: Unit = {
        out := in
        when(counter.notZero) {
          when(in === 0.U) {
            counter.dec()
            drop()
          }.otherwise{
...
// object WriteBufferLastEmitter extends App {
//   emitVerilog(
//     new WriteBufferLast(
