...
class LegacySpawnerServerIO(taskWidth: Int, regBlock: RegisterBlock, axiCfg : axi4.Config) extends Bundle {
...
class LegacySpawnerServer(
  ...
  val io = IO(new LegacySpawnerServerIO(taskWidth, regBlock, axiCfg))
...
object LegacySpawnerServerEmitter extends App {
  emitVerilog(new LegacySpawnerServer(256), Array("--target-dir", "output"))
}
