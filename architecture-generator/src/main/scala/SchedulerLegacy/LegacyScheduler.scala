...
import AXIHelpers._
import Util._
import Scheduler.{SchedulerAxiIO, SchedulerModule, SchedulerPEIO}
...
class LegacyScheduler(
...
) extends Module with SchedulerModule {

  require(!mfpgaSupport, "legacy architecture from 2469686 does not support the current mFPGA adapter")
  val fpgaCountInputReg: Option[UInt] = None
  val fpgaIndexInputReg: Option[UInt] = None
  val numberOfTasksToMove: Option[UInt] = None
  val m_axis_remote: Option[axi4s.Interface] = None
  val s_axis_remote: Option[axi4s.Interface] = None
  val s_axi_remote_task_server: Option[axi4.lite.Interface] = None
  val io_globalRun: Option[Bool] = None
  ...
  val spawnerServer = if(outsideSpawn) Some(Seq.fill(spawnerServerNumber)(
    Module(new LegacySpawnerServer(taskWidth, spawnerQueueDepth))
  )) else None
  
  val spawnerServerMgmt = if(outsideSpawn) Some(Seq.fill(spawnerServerNumber)(IO(axi4.lite.Slave(spawnerServer.get(0).asInstanceOf[LegacySpawnerServer].regBlock.cfgAxi))) ) else None
  val spawnerServerAXI = if(outsideSpawn) Some(Seq.fill(spawnerServerNumber)(IO(axi4.full.Master(spawnerServer.get(0).asInstanceOf[LegacySpawnerServer].axiCfg))) ) else None  
  override def legacySpawnerMgmt: Seq[axi4.lite.Interface] = spawnerServerMgmt.toSeq.flatten
  override def legacySpawnerAxi: Seq[axi4.full.Interface] = spawnerServerAXI.toSeq.flatten
  ...
  if(outsideSpawn){
    for(i <- 0 until spawnerServerNumber){
      spawnerServer.get(i).asInstanceOf[LegacySpawnerServer].io.axi_mgmt <> spawnerServerMgmt.get(i)
      spawnerServer.get(i).asInstanceOf[LegacySpawnerServer].io.m_axi.asFull :=> spawnerServerAXI.get(i)
      spawnerServer.get(i).asInstanceOf[LegacySpawnerServer].io.connNetwork_slave <> getOutsideSpawnNetwork.get.io.connSS(spawnerIndicies(i))
  ...
  if(outsideSpawn) {
    for(i <- 0 until spawnerServerNumber){
      spawnerServer.get(i).asInstanceOf[LegacySpawnerServer].io.connNetwork_master <> stealNW_TQ.io.connVAS(i)
    }
  }
  ...
  schedulerServers.zip(io_congested).foreach { case (server, tap) => tap := server.io.congested }
  ...
  val vssAdapter = Seq.fill(schedulerServersNumber)(
    Module(new LegacySchedulerAXIAdapter(taskWidth, addrWidth))
  )
  ...
  for (i <- 0 until peCount) {
    ...
    if (spawnsItself) {
      ...
      // The historical code used DontCare here. With valid held low this value
      // is unobservable; drive zero solely to satisfy current FIRRTL's stricter
      // initialization check.
      stealNW_TQ.io.connPE(i).push.bits := 0.U
    }
  ...
  if (peCountGlobalTaskIn > 0) {
    ...
    val globalsTaskBuffers = Seq.fill(peCountGlobalTaskIn)(
      Module(new LegacyGlobalTaskBuffer(taskWidth, peCount))
    )
